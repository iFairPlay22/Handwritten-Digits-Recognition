...
# Lancer l'execution en GPU (+ rapide) ou en CPU
GPU = True
if GPU:
...
class Network(object):
    ...
    def __init__(self, layers, batch_size):
        ...
        a value for the `batch_size` to be used during training
        by stochastic gradient descent.

        """
        # On set les paramètres du réseau
        self.layers = layers
    ...
    def SGD(self, training_data : list, epochs : int, batch_size : int, eta : float, validation_data : list, test_data : list, lmbda : float, learningCostPlot: SmartPlot, validationAccuracyPlot : SmartPlot, testAccuracyPlot : SmartPlot, color: str):
        """Train the network using batch stochastic gradient descent."""

        # On stocke les datasets dans des variables
        ...
        best_validation_accuracy_test_accuracy = 0
        best_validation_accuracy_training_cost = 0

        best_test_accuracy = 0.0
        best_test_accuracy_epoch = 0
        best_test_accuracy_validation_accuracy = 0
        best_test_accuracy_training_cost = 0
        ...
        for epoch in range(1, epochs + 1):
            ...
            # Pour chaque bach
            for batch_index in range(num_training_batches):

                # On entraine l'IA
                cost_ij = train_mb(batch_index)
                learningCosts.append(cost_ij)

            # On calcule le cost sur les données d'apprentissage
            training_cost = np.mean(learningCosts)
            learningCostPlot.addPoint("Network {}".format(self.id), color, training_cost)

            # On calcule l'accuracy sur les données de validation
            validation_accuracy = np.mean([validate_mb_accuracy(j) for j in range(num_validation_batches)])
            validationAccuracyPlot.addPoint("Network {}".format(self.id), color, validation_accuracy) 

            # On calcule l'accuracy sur les données de test
            test_accuracy = np.mean([test_mb_accuracy(j) for j in range(num_test_batches)])
            testAccuracyPlot.addPoint("Network {}".format(self.id), color, test_accuracy) 

            # Meilleure accuracy de validation
            if validation_accuracy >= best_validation_accuracy:
                best_validation_accuracy = validation_accuracy
                best_validation_accuracy_epoch = epoch
                best_validation_accuracy_test_accuracy = test_accuracy
                best_validation_accuracy_training_cost = training_cost

            # Meilleure accuracy de test
            if test_accuracy >= best_test_accuracy:
                best_test_accuracy = test_accuracy
                best_test_accuracy_epoch = epoch
                best_test_accuracy_validation_accuracy = validation_accuracy
                best_test_accuracy_training_cost = training_cost
        ...
        # On affiche les résultats du fin de traitement
        print()
        print("Finished training network.")
        print("Best validation accuracy : {0:.2%} corresponding to epoch {1}".format(best_validation_accuracy, best_validation_accuracy_epoch))
        print("Best test accuracy accuracy : {0:.2%} corresponding to epoch {1}".format(best_test_accuracy, best_test_accuracy_epoch))

        return best_validation_accuracy, best_validation_accuracy_epoch, best_validation_accuracy_test_accuracy, best_validation_accuracy_training_cost, best_test_accuracy, best_test_accuracy_epoch, best_test_accuracy_validation_accuracy, best_test_accuracy_training_cost

#### Define layer types

# Couche de convolution, pour traiter l'image en entrée et l'étudier plus en détail
class ConvPoolLayer(object):
    ...
    def __init__(self, filter_shape, image_shape, poolsize=(2, 2),
        ...
        batch size, the number of input feature maps, the image
...
# Couche initiale du réseau de neurones
...
# Couche finale du réseau de neurones
